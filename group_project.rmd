...
total.salary_mean <- gender.separated_data$HD_SALARY %>% mean() %>% round(digits = 2) %>% prettyNum(big.mark=",", trim=TRUE, digits = 7, scientific = FALSE)
total.salary_sd <- gender.separated_data$HD_SALARY %>% sd() %>% round(digits = 2) %>% prettyNum(big.mark=",", trim=TRUE, digits = 8, scientific = FALSE)
...
) %>% kable(booktabs = T) -> table.text
```
`r table.text`
```{r categorical variables description}
institution_data %>% group_by("Classification Name" = classification_name) %>% summarize(Count = n()) %>%
  mutate(Percentage = percent(Count / 1363)) %>% kable(booktabs = T)
```

```{r describe head coach salaries, message=FALSE}
gender.separated_data %>%
  ggplot(aes(x = HD_SALARY, fill = Gender)) +
