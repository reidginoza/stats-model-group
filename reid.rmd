...
```{r redo import}
institution_data.v2 <- read_spss('InstLevel.sav') %>%
...
institution_data %>%
  ...
  summarize(Mean.Men.Head.Coach=mean(HDCOACH_SALARY_MEN), SD.Men.Head.Coach=sd(HDCOACH_SALARY_MEN), Count=n()) # %>%
  # kable() %>%
...
institution_data %>%
  select(ClassificationCode, classification_name) %>%
  unique()
```
...
institution_data %>%
  ...
  select(classification_name,HDCOACH_SALARY_WOMEN) %>%
...
```{r}
adv_aov.results <- aov(Salary ~ classification_name*Gender, data = two.way.anova.attempt)
